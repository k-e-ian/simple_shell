...
int call_args(char **args, char **start, int *exe_ret);
int run_args(char **args, char **start, int *exe_ret);
...
int call_args(char **args, char **start, int *exe_ret)
{
	...
	for (index = 0; args[index]; index++)
	{
		if (_strncmp(args[index], "||", 2) == 0)
		{
			...
			ret = run_args(args, start, exe_ret);
			if (*exe_ret != 0)
			{
				args = &args[++index];
				index = 0;
...
int handle_args(int *exe_ret)
{
	...
	for (index = 0; args[index]; index++)
	{
		if (_strncmp(args[index], ";", 1) == 0)
		{
			free(args[index]);
			args[index] = NULL;
			ret = call_args(args, start, exe_ret);
			args = &args[++index];
			index = 0;
		}
	}
